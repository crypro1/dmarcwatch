...
# --- compute_dmarc_readiness() ---
#
# days=90 in den meisten Tests unten, unabhängig vom tatsächlichen
# Volumen - 90 Tage übertreffen selbst den strengsten Richtwert (60 Tage
# bei sehr wenig Volumen), sodass die neue Mindestbeobachtungsdauer diese
# Tests nicht verdeckt, die eigentlich own_ip_auth_failures/aktuelle
# Policy/Domain-Gruppierung prüfen. Die Mindestbeobachtungsdauer selbst
# wird weiter unten gezielt mit einem kurzen Zeitraum getestet.
...
def test_dmarc_readiness_ready_when_only_unknown_ip_failures():
    ...
    result = compute_dmarc_readiness(rows, days=90)
    assert len(result) == 1
    r = result[0]
...
def test_dmarc_readiness_empty_without_rows():
    assert compute_dmarc_readiness([], days=30) == []
...
def test_dmarc_readiness_not_ready_when_observation_window_too_short_for_volume():
    ...
    (hier 1 Eintrag über 7 Tage, also weit unter 1/Tag) empfiehlt
    _recommended_observation_days 60 Tage - 7 reichen nicht, obwohl keine
    einzige eigene IP fehlschlägt."""
    rows = [_row(DAY1, policy_p="quarantine")]
    result = compute_dmarc_readiness(rows, days=7)
...
def test_dmarc_readiness_ready_once_observation_window_matches_volume():
    rows = [_row(DAY1, policy_p="quarantine")]
    result = compute_dmarc_readiness(rows, days=60)
...
def test_mta_sts_readiness_no_data():
    result = compute_mta_sts_readiness([], days=30)
    assert result.has_data is False
    assert result.ready_for_enforce is False


def test_mta_sts_readiness_ready_with_zero_failures():
    # 10 Policies x 10 erfolgreiche Sitzungen = 100 Sitzungen über 14 Tage
    # -> ca. 7.1/Tag, damit klar oberhalb der 5/Tag-Schwelle ("hohes
    # Volumen", 14 Tage empfohlen) - 14 Tage Beobachtung reichen dann.
    result = compute_mta_sts_readiness([_tls_row(0)] * 10, days=14)
...
def test_mta_sts_readiness_not_ready_with_failures():
    result = compute_mta_sts_readiness([_tls_row(0), _tls_row(3)], days=90)
    assert result.total_failure_count == 3
    assert result.ready_for_enforce is False


def test_mta_sts_readiness_not_ready_when_observation_window_too_short_for_volume():
    # 1 Policy = 10 Sitzungen über 7 Tage -> ca. 1.4/Tag ("mittleres
    # Volumen", 30 Tage empfohlen) - 7 Tage reichen nicht.
    result = compute_mta_sts_readiness([_tls_row(0)], days=7)
    assert result.total_failure_count == 0
...
def test_to_stats_json_dict_structure():
    ...
    dmarc_readiness = compute_dmarc_readiness(rows, days=30)
    mta_sts_readiness = compute_mta_sts_readiness(tls_rows, days=30)
...
def test_to_stats_json_dict_tls_daily_defaults_to_empty():
    data = to_stats_json_dict(30, [], [], compute_mta_sts_readiness([], days=30))
